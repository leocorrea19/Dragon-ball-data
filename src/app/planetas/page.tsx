...
import { obtenerPlanetas } from '@/lib/api';
...
export default async function PlanetasPage({
  ...
  const { items: planetas, meta, errorPlanetas } = await obtenerPlanetas(paginaActual, limite);
  ...
  const crearUrlPagina = (pagina: number) => {
    ...
    return `/planetas?${params.toString()}`;
  };
    ...
    <div className="container mx-auto max-w-7xl px-4 py-16">
      <div className="flex flex-col items-center text-center">
        ...
        <p className="mt-4 max-w-2xl text-lg text-muted-foreground md:text-xl">
          Cada mundo tiene su propia historia.
        </p>
      </div>
      {errorPlanetas === false ? (
        <>
        ...
        <>
          <Alert variant="destructive" className='mt-12'>
            ...
            <AlertDescription>
              No se pudo obtener la lista de planetas. Por favor, intenta de nuevo más tarde.
            </AlertDescription>
          </Alert>
        </>
      )}
